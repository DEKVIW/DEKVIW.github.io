document.addEventListener("DOMContentLoaded", function () {
  ...
  const rightSideFn = {
    switchReadMode: () => {
      ...
      console.log("阅读模式切换被触发"); // 调试信息

      // 检查是否已经在阅读模式中，如果是则退出
      if ($body.classList.contains("read-mode")) {
        console.log("退出阅读模式"); // 调试信息
        $body.classList.remove("read-mode");
        const existingBtn = document.querySelector(".exit-readmode");
        if (existingBtn) {
          existingBtn.remove();
        }
      ...
      const stopAllAnimations = () => {
        const spinElements = document.querySelectorAll(
          ".fa-spin, .fa-spinner, i.fa-spin, i.fa-spinner"
        );
      ...
      setTimeout(stopAllAnimations, 200);
      setTimeout(stopAllAnimations, 500);
    },
    switchDarkMode: () => {
